...
namespace UnifiedDataExplorer.ViewModel
{
    public class PiJsonDisplayViewModel : ExplorePointViewModel
    {
        ...
        {
            ItemBase itemBase;
            if (loadingInfo.Tag == ServerDatabaseAssetWrapper.ASSET_SERVER_TYPE) itemBase = await _client.GetBySelfLink<AssetServer>(loadingInfo.Id);
            else if (loadingInfo.Tag == ServerDatabaseAssetWrapper.DATABASE_TYPE) itemBase = await _client.GetBySelfLink<Database>(loadingInfo.Id);
            else if (loadingInfo.Tag == ServerDatabaseAssetWrapper.ASSET_TYPE) itemBase = await _client.GetBySelfLink<Asset>(loadingInfo.Id);
